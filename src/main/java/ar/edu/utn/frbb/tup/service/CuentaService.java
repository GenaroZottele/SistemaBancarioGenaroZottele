...
public class CuentaService {
    ...
    public Cuenta darDeAltaCuenta(Cuenta cuenta, long dniTitular)
        ...
        //Verificar si el tipo de cuenta es soportado
        if (!esTipoDeCuentaSoportado(cuenta.getTipoCuenta().toString())) {
            throw new TipoCuentaNotSupportedException("El tipo de cuenta no es soportado.");
        }

        // Buscar cliente real (en vez de new Cliente())
        ...
        // Verificar si el cliente ya tiene una cuenta con el MISMO tipo Y la MISMA moneda
        ...
        if (existeMismoTipoYMoneda) {
            throw new CuentaAlreadyExistsException("El cliente ya tiene una cuenta de este tipo y moneda.");
        }

        //Vincular la cuenta con el titular usando el método de ClienteService 
    ...
    public Cuenta findCuentaById(long id) {
        ...
        //Si la cuenta tiene un DNI de titular, cargamos el cliente desde ClienteService
